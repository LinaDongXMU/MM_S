...
def _read_mol(fmt, filename):
    """
    Compatible wrapper for pybel molecule reading
    """
    try:
        # old pybel
        return pybel.readfile(fmt, filename).__next__()
    except AttributeError:
        # new openbabel pybel
        return next(pybel.readfile(fmt, filename))
...
def runMSMS(inprot, inlig, MSMSDIR = '.'):
    ...
    if intype[1:].lower() != 'pdb':
        prot = _read_mol(intype[1:], inprot)
